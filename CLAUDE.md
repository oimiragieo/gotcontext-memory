# CLAUDE.md — gotcontext-memory

Follow [`AGENTS.md`](./AGENTS.md). Same rules bind Claude Code, Cursor, Codex, and other agents.
...
- `gotcontext-memory-hitl-honesty` — HITL / locks / MCP / inert tests / no false omega parity  
- `gotcontext-memory-streaming-digests` — scale, digests, stratified window, `.vscdb`, concurrency  
- `gotcontext-memory-claim-lifecycle` — `claimKey`, prevalence, evidence sort, preference FPs  

Also: [`docs/features/efficacy.md`](./docs/features/efficacy.md) after accepting pattern notes.
