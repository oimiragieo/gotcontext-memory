...
| BL-DRM-022 | Weekly CONSOLIDATION (region-rewrite) tier | OPEN | Re-read one ragged memory region; merge overlaps, supersede contradicted notes, expire mechanized-RESOLVED ones; survival requires re-synthesis (Auto-Dreamer 2605.20616). Emit per-note CAS-gated proposals, never one giant write. **Proven deterministic parameters to port** (gbrain MIT, `src/core/cycle/phases/consolidate.ts`, read 2026-08-18): bucket unconsolidated items per (source, entity); skip if count < 3 OR oldest < 24h; greedy cosine cluster at 0.85; promote the highest-confidence member as the consolidated claim; NEVER DELETE — stamp `consolidated_into` pointers on members. Deterministic (no LLM) keeps the cycle reproducible; an LLM rewrite is an optional later stage. Also their phase-order principle: fix files first, then index. |
| BL-DRM-026 | Cycle lock + orphan sweep (gbrain-proven; live in the Python engine 2026-08-18) | OPEN | (a) Single-writer lock for any daemon/cron dream: pid+timestamp lockfile, 30-min TTL, PID liveness, stale-steal; refuse-not-queue on a live holder. **Windows trap for any port: `kill(pid, 0)` TERMINATES the target — probe via OpenProcess.** (b) Report-only orphan sweep: this store's index is regenerated so disk-vs-index orphans cannot occur, but dangling accepted-proposal receipts and files outside `memoryTreeHash` coverage can — sweep and report, never auto-fix. First live run of (b) on the Python engine found **896 unindexed notes** — the systemic cause of the reads_post=0 epidemic. |
