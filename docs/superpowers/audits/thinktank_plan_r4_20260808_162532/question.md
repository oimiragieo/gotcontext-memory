# Thinktank R4 final gate — gotcontext-memory plan

Mandate: Answer inline. No agents. Prefer APPROVE if no *new blocking* contradiction.
Checklist (all should be PRESENT): commitCanonical/Operational/deleteCanonical; withCanonicalLocks; regenerateIndex overlay; memoryTreeHash; CE-8; expire/delete; proposal secrets; proper-lockfile cross-process; path containment; initStore + installer runtime arm; portability external-archive allowlist; node:sqlite read-only Cursor corpus; primary bin gotcontext-memory; dream = zero canonical writes.

End with: RECOMMENDED: APPROVE | APPROVE_WITH_FIXES | REJECT — <≤20 words>
...
PLAN_STATUS: APPROVED_PENDING_QUICK_CONFIRM
